...
export class TodoController {
  ...
  public async show(req: express.Request, res: express.Response) {
    const todo = await this._todoModel.findOne(Number(req.params.id))
  ...
  public async create(req: express.Request, res: express.Response) {
    const todo = await this._todoModel.buildNewTodo(req.body)
  ...
  public async update(req: express.Request, res: express.Response) {
    let todo = await this._todoModel.findOne(Number(req.params.id))
    if (!todo) {
      return res.status(404).send("このTODOは存在しません")
    }
